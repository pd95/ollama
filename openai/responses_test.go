...
func TestFromResponsesRequest_NestedNamespaceTools(t *testing.T) {
	req := ResponsesRequest{
		Model: "gpt-oss:20b",
		Input: ResponsesInput{Text: "hello"},
		Tools: []ResponsesTool{
				...
				Tools: []ResponsesTool{
					{
						Type:  "namespace",
						Name:  "inner",
						Tools: []ResponsesTool{{Type: "function", Name: "leaf"}},
					},
				},
			},
		},
	...
	chatReq, err := FromResponsesRequest(req)
	if err != nil {
		t.Fatalf("failed to convert nested namespaces: %v", err)
	}
	if len(chatReq.Tools) != 1 {
		t.Fatalf("expected 1 converted tool, got %d", len(chatReq.Tools))
	}
	if got := chatReq.Tools[0].Function.Name; got != "outer__inner__leaf" {
		t.Errorf("flattened name = %q, want %q", got, "outer__inner__leaf")
	}
}
...
func TestToResponse_NestedNamespaceToolCall(t *testing.T) {
	response := ToResponse("gpt-oss:20b", "resp_123", "msg_456", api.ChatResponse{
		CreatedAt: time.Now(),
	...
	item := response.Output[0]
	if item.Name != "leaf" || item.Namespace != "outer__inner" {
		t.Errorf("tool call = namespace %q name %q, want namespace %q name %q", item.Namespace, item.Name, "outer__inner", "leaf")
	}
}
