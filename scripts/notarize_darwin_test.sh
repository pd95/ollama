...
assert_contains "$SCRIPT_DIR/build_darwin.sh" './scripts/notarize_darwin.sh'
assert_contains "$SCRIPT_DIR/build_darwin.sh" '_sign_app_bundle "$APPLE_IDENTITY"'
if grep -F 'notarytool) submit dist/Ollama-darwin.zip' "$SCRIPT_DIR/build_darwin.sh" >/dev/null; then
  fail 'Darwin app packaging still submits directly instead of using resumable state'
fi
pass 'Darwin app packaging routes Developer ID signing through resumable notarization'
