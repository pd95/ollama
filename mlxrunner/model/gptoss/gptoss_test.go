...
const row11NoSinkSentinel float32 = -1e9

func forwardModel(m *Model, tokens *mlx.Array, caches []cache.Cache) *mlx.Array {
	hidden, _ := m.Forward(testBatch(tokens, 0), caches)
	return hidden
}
...
func row11SliceSequence(x *mlx.Array, pos int) *mlx.Array {
	return x.Slice(mlx.Slice(), mlx.Slice(pos, pos+1), mlx.Slice())
}
...
func refreshRow11CheckedConfig(t *testing.T, cfg *Config) {
	t.Helper()
	checked, err := validateGPTOSSConfigDimensions(cfg)
	if err != nil {
		t.Fatalf("validate mutated GPT-OSS config: %v", err)
	}
	cfg.checked = checked
}
...
func TestNewModelRestoresHarmonyStopTokens(t *testing.T) {
	...
	want := []int32{200002, 199999, 199999, 200002, 200012}
...
func TestLoadWeightsDensePath(t *testing.T) {
	...
	for i, layer := range m.Layers {
		...
		if layer.Attention.Sinks.DType() != mlx.DTypeBFloat16 {
			t.Fatalf("layer %d Attention.Sinks dtype = %s, want BF16 prepared state", i, layer.Attention.Sinks.DType())
		}
...
func TestLoadWeightsRejectsUnsupportedQuantizedDirectExperts(t *testing.T) {
	t.Run("affine-int8", func(t *testing.T) {
		testLoadWeightsRejectsUnsupportedQuantizedDirectExperts(t, 64, 8, "affine")
	})
	t.Run("nvfp4", func(t *testing.T) {
		testLoadWeightsRejectsUnsupportedQuantizedDirectExperts(t, 16, 4, "nvfp4")
	})
}

func testLoadWeightsRejectsUnsupportedQuantizedDirectExperts(t *testing.T, groupSize, bits int, mode string) {
	skipIfNoMLX(t)
	...
	tensors := denseTestTensors(t, cfg)
	projection := "blocks.0.experts.gate_proj.weight"
	quantizeTensorForTest(t, tensors, projection, groupSize, bits, mode)
	err := m.LoadWeights(tensors)
	if err == nil {
		t.Fatal("LoadWeights() error = nil, want unsupported direct expert quantization rejection")
	}
	want := fmt.Sprintf("tensor %q quantization = %s/%d/group%d, want mxfp4/4/group32", projection, mode, bits, groupSize)
	if !strings.Contains(err.Error(), want) {
		t.Fatalf("LoadWeights() error = %q, want containing %q", err, want)
	}
}
...
func TestLoadWeightsExpertMissingTensorFails(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "layer 0") || !strings.Contains(err.Error(), `"blocks.0.experts.down_proj"`) || !strings.Contains(err.Error(), "incomplete direct expert tensors") {
...
func TestBuildGPTOSSRoPEFreqsMatchesReference(t *testing.T) {
	...
	refreshRow11CheckedConfig(t, &cfg)
...
func TestForwardRunsCompletePath(t *testing.T) {
	...
	if err := m.LoadWeights(denseRuntimeTestTensors(t, cfg)); err != nil {
		t.Fatalf("LoadWeights() error = %v", err)
	}
	caches := m.NewCaches()
	tokens := mlx.FromValues([]int32{1, 2}, 1, 2)
	out, aux := m.Forward(testBatch(tokens, 0), caches)

	if out == nil || !out.Valid() {
		t.Fatal("Forward() returned invalid output")
	}
	if aux == nil || !aux.Valid() || aux != out {
		t.Fatal("Forward() did not return the output as draft-conditioning state")
	}
...
func assertQuantizedAttentionCachedPrefillParity(
	...
	for pos := range seqLen {
		_, stepTrace = attentionForwardTraceForTest(t, attn, row11SliceSequence(x, pos), stepCache, 1, 1, cfg, 0)
	}
...
func TestQuantizedLayerLastTokenMatchesCachedPrefillStepPath(t *testing.T) {
	...
	var step *mlx.Array
	for pos := range seqLen {
		step = layer.Forward(row11SliceSequence(x, pos), stepCache, 1, 1, &cfg, 0)
	}
...
func TestAttentionLastTokenMatchesPrefillStepPathScaledRoPE(t *testing.T) {
	...
	attn := &Attention{
		...
		Sinks:     preparedSyntheticSink(t, []float32{0.3}, mlx.DTypeFloat32),
...
func TestAttentionForwardMatchesReference(t *testing.T) {
	...
	attn := &Attention{
		...
		Sinks: preparedSyntheticSink(t, []float32{row11NoSinkSentinel}, mlx.DTypeFloat32),
	}
...
func TestExpertsForwardMatchesExplicitRealImportedModel(t *testing.T) {
	...
	if err := baseModel.LoadWeights(tensors); err != nil {
...
func denseTestTensors(t *testing.T, cfg Config) map[string]*mlx.Array {
	...
	for i := range cfg.NumHiddenLayers {
		...
		tensors[prefix+".attn_sinks"] = denseVector(int(cfg.NumAttentionHeads), 12+float32(i)).AsType(mlx.DTypeBFloat16)
...
// denseRuntimeTestTensors models the BF16 checkpoint used by the production
// GPT-OSS path. Keeping denseTestTensors in its original dtype lets loader-only
// tests continue to vary individual tensor contracts independently.
func denseRuntimeTestTensors(t *testing.T, cfg Config) map[string]*mlx.Array {
	t.Helper()
	tensors := denseTestTensors(t, cfg)
...
// prepareSyntheticF32Sinks adapts direct synthetic layer/SDPA fixtures whose
// deliberately F32 projections bypass the model-load/runtime BF16 contract.
func prepareSyntheticF32Sinks(t *testing.T, m *Model) {
	t.Helper()
	for i, layer := range m.Layers {
		if layer == nil || layer.Attention == nil || layer.Attention.Sinks == nil {
			t.Fatalf("layer %d is missing attention sinks", i)
		}
		values := materializedFloats(layer.Attention.Sinks.AsType(mlx.DTypeFloat32))
		layer.Attention.Sinks = preparedSyntheticSink(t, values, mlx.DTypeFloat32)
	}
}

func preparedSyntheticSink(t *testing.T, values []float32, dtype mlx.DType) *mlx.Array {
	t.Helper()
	prepared := mlx.Contiguous(mlx.FromValues(values, len(values)).AsType(dtype), false)
	mlx.Eval(prepared)
	return prepared
}
...
func testRoot(t *testing.T, configJSON, tokenizerJSON []byte) *model.Root {
	t.Helper()
	return testRootWithExtraConfigs(t, configJSON, tokenizerJSON, nil)
}
...
func testRootWithExtraConfigs(t *testing.T, configJSON, tokenizerJSON []byte, extraConfigs map[string][]byte) *model.Root {
	...
	writeConfig := func(name string, content []byte) manifest.Layer {
		t.Helper()
		...
		return manifest.Layer{MediaType: "application/vnd.ollama.image.json", Digest: digest, Name: name, Size: int64(len(content))}
	}

	layers := []manifest.Layer{
		writeConfig("config.json", configJSON),
		writeConfig("tokenizer.json", tokenizerJSON),
	}
	for name, content := range extraConfigs {
		layers = append(layers, writeConfig(name, content))
	}
	return &model.Root{Manifest: &manifest.Manifest{
		SchemaVersion: 2,
