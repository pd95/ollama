...
func init() {
	model.Register("GptOssForCausalLM", NewModel)
}
...
type Config struct {
	...
	checked        gptossCheckedDimensions           `json:"-"`
}

const (
	gptossMaxLoopEntries = 1 << 20
	gptossMaxDimension   = 1 << 24
)

type gptossCheckedDimensions struct {
	layers, experts, expertsPerToken int
	hidden, intermediate, headDim    int
	attentionHeads, kvHeads          int
	vocab, slidingWindow, context    int
	q, kv, gateUp                    int
}

func checkedGPTOSSProduct(limit uint64, values ...int32) (int, error) {
	product := uint64(1)
	for _, value := range values {
		if value <= 0 || product > ^uint64(0)/uint64(value) {
			return 0, fmt.Errorf("invalid dimension product %v", values)
		}
		product *= uint64(value)
	}
	if product > limit || product > uint64(^uint(0)>>1) {
		return 0, fmt.Errorf("dimension product %v = %d exceeds limit %d", values, product, limit)
	}
	return int(product), nil
}

func validateGPTOSSConfigDimensions(cfg *Config) (gptossCheckedDimensions, error) {
	if cfg == nil {
		return gptossCheckedDimensions{}, fmt.Errorf("missing config")
	}
	loopScalars := []struct {
		name  string
		value int32
		...
		{"num_local_experts", cfg.NumLocalExperts},
	...
	for _, scalar := range loopScalars {
		if scalar.value <= 0 || scalar.value > gptossMaxLoopEntries {
			return gptossCheckedDimensions{}, fmt.Errorf("%s = %d exceeds supported range 1..%d", scalar.name, scalar.value, gptossMaxLoopEntries)
		}
	}
	dimensionScalars := []struct {
		name  string
		value int32
	}{
		...
		{"rope_scaling.original_max_position_embeddings", cfg.RopeScaling.OriginalMaxPositionEmbeddings},
	}
	for _, scalar := range dimensionScalars {
		if scalar.value <= 0 || scalar.value > gptossMaxDimension {
			return gptossCheckedDimensions{}, fmt.Errorf("%s = %d exceeds supported range 1..%d", scalar.name, scalar.value, gptossMaxDimension)
		}
	}
	if cfg.RopeTheta <= 0 || cfg.RMSNormEps <= 0 || cfg.RopeScaling.Factor <= 0 {
		return gptossCheckedDimensions{}, fmt.Errorf("rope_theta, rms_norm_eps, and rope_scaling.factor must be positive")
	}
	if cfg.NumAttentionHeads%cfg.NumKeyValueHeads != 0 {
		return gptossCheckedDimensions{}, fmt.Errorf("num_attention_heads must be divisible by num_key_value_heads")
	}
	if cfg.NumExpertsPerTok > cfg.NumLocalExperts {
		return gptossCheckedDimensions{}, fmt.Errorf("num_experts_per_tok must not exceed num_local_experts")
	}
	q, err := checkedGPTOSSProduct(gptossMaxDimension, cfg.NumAttentionHeads, cfg.HeadDim)
	if err != nil {
		return gptossCheckedDimensions{}, fmt.Errorf("query dimension: %w", err)
	}
	kv, err := checkedGPTOSSProduct(gptossMaxDimension, cfg.NumKeyValueHeads, cfg.HeadDim)
	if err != nil {
		return gptossCheckedDimensions{}, fmt.Errorf("key/value dimension: %w", err)
	}
	gateUp, err := checkedGPTOSSProduct(gptossMaxDimension, 2, cfg.IntermediateSize)
	if err != nil {
		return gptossCheckedDimensions{}, fmt.Errorf("gate/up dimension: %w", err)
	}
	if _, err := checkedGPTOSSProduct(gptossMaxLoopEntries, cfg.NumHiddenLayers, cfg.NumLocalExperts); err != nil {
		return gptossCheckedDimensions{}, fmt.Errorf("layer/expert work: %w", err)
	}
	return gptossCheckedDimensions{
		layers: int(cfg.NumHiddenLayers), experts: int(cfg.NumLocalExperts), expertsPerToken: int(cfg.NumExpertsPerTok),
		hidden: int(cfg.HiddenSize), intermediate: int(cfg.IntermediateSize), headDim: int(cfg.HeadDim),
		attentionHeads: int(cfg.NumAttentionHeads), kvHeads: int(cfg.NumKeyValueHeads), vocab: int(cfg.VocabSize),
		slidingWindow: int(cfg.SlidingWindow), context: int(cfg.MaxPositionEmbeddings), q: q, kv: kv, gateUp: gateUp,
	}, nil
}
...
type fusedExpertDescriptor struct {
	present, hasQBias, transpose    bool
	mode                            string
	bits, groupSize                 int
	weightType, scaleType, biasType mlx.DType
	weightDims, scaleDims, biasDims []int
}

func fusedExpertDescriptorFor(p *ExpertProjection) fusedExpertDescriptor {
	if p == nil || p.Weight == nil || !p.Weight.Valid() || p.Scales == nil || !p.Scales.Valid() || p.Bias == nil || !p.Bias.Valid() {
		return fusedExpertDescriptor{}
	}
	return fusedExpertDescriptor{
		present: true, hasQBias: p.QBiases != nil, transpose: p.Transpose, mode: p.Mode, bits: p.Bits, groupSize: p.GroupSize,
		weightType: p.Weight.DType(), scaleType: p.Scales.DType(), biasType: p.Bias.DType(),
		weightDims: p.Weight.Dims(), scaleDims: p.Scales.Dims(), biasDims: p.Bias.Dims(),
	}
}

func fusedExpertDescriptorEligible(d fusedExpertDescriptor) bool {
	return d.present && !d.hasQBias && d.mode == "mxfp4" && d.bits == 4 && d.groupSize == 32 && d.transpose &&
		d.weightType == mlx.DTypeUint32 && d.scaleType == mlx.DTypeUint8 && d.biasType == mlx.DTypeBFloat16 &&
		len(d.weightDims) == 3 && len(d.scaleDims) == 3 && len(d.biasDims) == 2
}

func fusedExpertProjectionEligible(p *ExpertProjection) bool {
	return fusedExpertDescriptorEligible(fusedExpertDescriptorFor(p))
}

func fusedExpertPairEligible(gate, up *ExpertProjection) bool {
	g, u := fusedExpertDescriptorFor(gate), fusedExpertDescriptorFor(up)
	return fusedExpertDescriptorsPairEligible(g, u)
}

func fusedExpertDescriptorsPairEligible(g, u fusedExpertDescriptor) bool {
	return fusedExpertDescriptorEligible(g) && fusedExpertDescriptorEligible(u) &&
		slices.Equal(g.weightDims, u.weightDims) && slices.Equal(g.scaleDims, u.scaleDims) && slices.Equal(g.biasDims, u.biasDims)
}
...
func NewModel(root *model.Root) (model.Model, error) {
	...
	return &Model{
		Config: &cfg,
		Layers: make([]*Layer, cfg.checked.layers),
		tok:    tok,
	}, nil
}
...
func parseConfig(configData []byte) (Config, error) {
	...
	checked, err := validateGPTOSSConfigDimensions(&cfg)
	if err != nil {
		return Config{}, err
	}
	cfg.checked = checked
...
func (m *Model) NumLayers() int {
	...
	if m.checked.layers > 0 {
		return m.checked.layers
	}
...
func prepareGPTOSSAttentionSinks(layer int, name string, source *mlx.Array, heads int) (*mlx.Array, error) {
	if err := validateLayerTensorShape(layer, name, source, []int{heads}, "num_attention_heads"); err != nil {
		return nil, err
	}
	if err := validateLayerTensorDType(layer, name, source, mlx.DTypeBFloat16, "GPT-OSS checkpoint attention sinks"); err != nil {
		return nil, err
	}
	prepared := mlx.Contiguous(source, false)
	if prepared == nil || !prepared.Valid() {
		return nil, fmt.Errorf("layer %d: failed to prepare tensor %q as contiguous BF16", layer, name)
	}
	mlx.Eval(prepared)
	if err := validateLayerTensorShape(layer, name+" (prepared)", prepared, []int{heads}, "prepared num_attention_heads"); err != nil {
		return nil, err
	}
	if err := validateLayerTensorDType(layer, name+" (prepared)", prepared, mlx.DTypeBFloat16, "prepared GPT-OSS attention sinks"); err != nil {
		return nil, err
	}
	finiteProbe := prepared.AsType(mlx.DTypeFloat32)
	mlx.Eval(finiteProbe)
	for i, value := range finiteProbe.Floats() {
		if math.IsNaN(float64(value)) || math.IsInf(float64(value), 0) {
			return nil, fmt.Errorf("layer %d: tensor %q contains non-finite value at %d", layer, name, i)
		}
	}
	return prepared, nil
}

func validateGPTOSSAttentionSinkDispatch(query, sinks *mlx.Array, heads int) error {
	if query == nil || !query.Valid() {
		return fmt.Errorf("query must be a valid rank-4 array")
	}
	if query.NumDims() != 4 {
		return fmt.Errorf("query must be rank-4, got shape %v dtype %s", query.Dims(), query.DType())
	}
	if query.Dim(1) != heads {
		return fmt.Errorf("query head axis = %d, want configured heads = %d (shape %v)", query.Dim(1), heads, query.Dims())
	}
	if sinks == nil || !sinks.Valid() {
		return fmt.Errorf("prepared sinks must be a valid rank-1 array with %d heads", heads)
	}
	if sinks.NumDims() != 1 || sinks.Dim(0) != heads {
		return fmt.Errorf("prepared sinks must be rank-1 with %d heads, got shape %v dtype %s", heads, sinks.Dims(), sinks.DType())
	}
	if sinks.DType() != query.DType() {
		return fmt.Errorf("prepared sink dtype %s must equal actual query dtype %s", sinks.DType(), query.DType())
	}
	return nil
}
...
func loadDirectExpertProjection(tensors map[string]*mlx.Array, layer int, prefix string, wantOut, wantIn int, cfg *Config) (*ExpertProjection, error) {
	...
	if weight == nil && bias == nil && scales == nil && qbiases == nil {
		return nil, nil
	}
	if weight == nil || bias == nil {
		return nil, fmt.Errorf("layer %d: incomplete direct expert tensors for %q", layer, prefix)
	}
	...
	if scales != nil {
		...
		if mode != "mxfp4" || bits != 4 || groupSize != 32 {
			return nil, fmt.Errorf("layer %d: tensor %q quantization = %s/%d/group%d, want mxfp4/4/group32", layer, weightName, mode, bits, groupSize)
		}
		if wantIn <= 0 || wantIn%32 != 0 {
			return nil, fmt.Errorf("layer %d: tensor %q input dim %d must be positive and divisible by 32", layer, weightName, wantIn)
		}
		if qbiases != nil {
			return nil, fmt.Errorf("layer %d: native MXFP4 tensor %q must not have quantization bias", layer, weightName)
		}
		if err := validateLayerTensorShape(layer, weightName, weight, []int{int(cfg.NumLocalExperts), wantOut, wantIn / 8}, "experts x out x packed-input words"); err != nil {
			return nil, err
		}
		if err := validateLayerTensorDType(layer, weightName, weight, mlx.DTypeUint32, "native MXFP4 packed weights"); err != nil {
			return nil, err
		}
		if err := validateLayerTensorShape(layer, weightName+"_scale", scales, []int{int(cfg.NumLocalExperts), wantOut, wantIn / 32}, "experts x out x MXFP4 groups"); err != nil {
			return nil, err
		}
		if err := validateLayerTensorDType(layer, weightName+"_scale", scales, mlx.DTypeUint8, "native MXFP4 scales"); err != nil {
			return nil, err
		}
	...
	if qbiases != nil {
		return nil, fmt.Errorf("layer %d: orphan quantization bias for %q", layer, weightName)
	}
...
func (m *Model) LoadWeights(tensors map[string]*mlx.Array) error {
	...
	checked, err := validateGPTOSSConfigDimensions(m.Config)
	if err != nil {
		return err
	}
	m.checked = checked
	...
	for i := range m.Layers {
		...
		sinkSource, err := requireTensor(tensors, prefix+".attn_sinks")
		if err != nil {
			return fmt.Errorf("layer %d: %w", i, err)
		}
		sinks, err := prepareGPTOSSAttentionSinks(i, prefix+".attn_sinks", sinkSource, checked.attentionHeads)
		if err != nil {
			return err
		}
		...
		gateUp, err := loadExpertPair(tensors, i, prefix+".experts.gate_up_proj", checked.gateUp, checked.hidden, m.Config)
		if err != nil {
			return err
		}
		down, err := loadExpertProjection(tensors, i, prefix+".experts.down_proj", checked.hidden, checked.intermediate, m.Config)
		if err != nil {
			return err
		}
		m.Layers[i] = &Layer{
...
func (m *Model) Forward(b *batch.Batch, caches []cache.Cache) (hidden, auxHidden *mlx.Array) {
	...
	out := m.forwardDense(b, caches, batchSize, seqLen)
	return out, out
}
...
func (a *Attention) ForwardBatch(x *mlx.Array, b *batch.Batch, c cache.Cache, batchSize, seqLen int, cfg *Config, layerIndex int) *mlx.Array {
	...
	if err := validateGPTOSSAttentionSinkDispatch(query, a.Sinks, int(cfg.NumAttentionHeads)); err != nil {
		panic(fmt.Sprintf("gpt-oss layer %d attention sink dispatch: %v", layerIndex, err))
	}
	...
	attention = mlx.Reshape(attention, batchDim, seq, int32(cfg.checked.q))
	if attention == nil || !attention.Valid() {
...
func (p *ExpertProjection) Forward(x, indices *mlx.Array, sorted bool) *mlx.Array {
	...
	var out *mlx.Array
	if p.Scales != nil {
		out = mlx.GatherQMM(x, p.Weight, p.Scales, p.QBiases, nil, indices, p.Transpose, p.GroupSize, p.Bits, p.Mode, nil, sorted)
	} else {
...
func (e *Experts) Forward(x, router *mlx.Array, cfg *Config, layerIndex int) *mlx.Array {
	...
	canFuse := !doSort && B*L == 1 &&
		fusedExpertPairEligible(e.GateUp.Gate, e.GateUp.Up)
	...
	if canFuse {
		...
		if len(wDims) == 3 {
			...
			if ok && swiGLUOut != nil {
				...
				if fusedExpertProjectionEligible(e.Down) {
