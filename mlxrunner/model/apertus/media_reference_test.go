...
func TestMediaCodeReference(t *testing.T) {
	...
	var reference struct {
		ImageCodes        []int32 `json:"image_codes"`
		ResizedImageCodes []int32 `json:"resized_image_codes"`
		AudioCodes        []int32 `json:"audio_codes"`
		ShortAudioCodes   []int32 `json:"short_audio_codes"`
	...
	defer func() {
		if err := thread.Stop(context.Background(), func() { mlx.ClearCache() }); err != nil {
			t.Fatal(err)
		}
	}()
...
func compareCodeIDs(name string, got, want []int32) error {
	if len(got) != len(want) {
