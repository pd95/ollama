...
func validMoEPositiveInt(v int) bool {
	return v > 0 && v <= math.MaxInt32
}

// checkedMoESpan returns an element span whose maximum zero-based index fits
// the uint32 index type used by the Metal kernels.
func checkedMoESpan(factors ...uint64) (uint64, bool) {
	span := uint64(1)
	for _, factor := range factors {
		if factor == 0 || span > math.MaxUint64/factor {
			return 0, false
		}
		span *= factor
	}
	if span > uint64(math.MaxUint32)+1 {
		return 0, false
	}
	return span, true
}

func validateMoEIndexSpans(experts, batch, numRows, numColVecs, topK int, down bool) bool {
	values := []int{experts, batch, numRows, numColVecs, topK}
	factors := make([]uint64, len(values))
	for i, value := range values {
		if !validMoEPositiveInt(value) {
			return false
		}
		factors[i] = uint64(value)
	}
	e, b, r, c, k := factors[0], factors[1], factors[2], factors[3], factors[4]

	// Complete spans prove expertStride, expert/row/channel terms, companion
	// offsets, selector offsets, and output offsets without uint32 wrapping.
	for _, product := range [][]uint64{
		{r, c},
		{e, r, c},
		{e, r},
		{b, k},
		...
		if _, ok := checkedMoESpan(product...); !ok {
			return false
		}
	}

	inputSpan := []uint64{b, c, 8}
	if down {
		inputSpan = []uint64{b, k, c, 8}
	}
	_, ok := checkedMoESpan(inputSpan...)
	return ok
}
...
func validMoEArray(a *Array, dtype DType, dims ...int) bool {
	...
	for i, dim := range dims {
		if !validMoEPositiveInt(dim) || a.Dim(i) != dim || size > math.MaxInt/dim {
			return false
		}
		size *= dim
	}
...
func validateMoEProjection(weight, scales, bias *Array, numRows, numColVecs int) (int, bool) {
	if weight == nil || scales == nil || bias == nil || !validMoEPositiveInt(numRows) || !validMoEPositiveInt(numColVecs) || numColVecs < 32 || numColVecs > math.MaxInt/32 || numColVecs > math.MaxInt/4 {
		return 0, false
	}
	dims := weight.Dims()
	if len(dims) != 3 || !validMoEPositiveInt(dims[0]) {
		return 0, false
	}
	experts := dims[0]
	if !validMoEArray(weight, DTypeUint32, experts, numRows, numColVecs*4) ||
		!validMoEArray(scales, DTypeUint8, experts, numRows, numColVecs) ||
		!validMoEArray(bias, DTypeBFloat16, experts, numRows) {
		return 0, false
	}
	// The Metal kernel is created with ensure_row_contiguous=true. Together
	// with these exact shapes and sizes, MLX supplies the linear row strides
	// used by the kernel instead of exposing unchecked source strides here.
	return experts, true
}

func validateMoEGateUpInputs(
	input, gateWeight, gateScales, gateBias,
	upWeight, upScales, upBias, expertIds *Array,
	numRows, numColVecs, topK int,
) (int, bool) {
	if !validMoEPositiveInt(topK) {
		return 0, false
	}
	experts, ok := validateMoEProjection(gateWeight, gateScales, gateBias, numRows, numColVecs)
	if !ok || topK > experts {
		return 0, false
	}
	upExperts, ok := validateMoEProjection(upWeight, upScales, upBias, numRows, numColVecs)
	if !ok || upExperts != experts {
		return 0, false
	}
	inDims := input.Dims()
	if len(inDims) != 2 || !validMoEPositiveInt(inDims[0]) {
		return 0, false
	}
	batch := inDims[0]
	if !validMoEArray(input, DTypeFloat32, batch, numColVecs*32) ||
...
func validateMoEDownInputs(input, weight, scales, bias, expertIds *Array, numRows, numColVecs, topK int) (int, bool) {
	if !validMoEPositiveInt(topK) {
		return 0, false
	}
	experts, ok := validateMoEProjection(weight, scales, bias, numRows, numColVecs)
	if !ok || topK > experts {
		return 0, false
	}
	inDims := input.Dims()
	if len(inDims) != 3 || !validMoEPositiveInt(inDims[0]) {
		return 0, false
	}
	batch := inDims[0]
	if !validMoEArray(input, DTypeFloat32, batch, topK, numColVecs*32) ||
		!validMoEArray(expertIds, DTypeUint32, batch, topK) {
		return 0, false
	}
...
func MoEFusedGateUpSwiGLU(
	...
	batch, ok := validateMoEGateUpInputs(input, gateWeight, gateScales, gateBias, upWeight, upScales, upBias, expertIds, numRows, numColVecs, topK)
	if !ok {
		return nil, false
	}
	...
	if numRows > math.MaxInt32/(32*nSg) {
		return nil, false
	}
...
func MoEFusedDown(
	...
	batch, ok := validateMoEDownInputs(input, downWeight, downScales, downBias, expertIds, numRows, numColVecs, topK)
	if !ok {
		return nil, false
	}
