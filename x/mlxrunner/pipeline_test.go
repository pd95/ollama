...
func TestPreparePassesLiveContextAndPublishesNothingWhenCanceled(t *testing.T) {
	...
	failed.Format = []byte(`{"type":"object"}`)
	r.grammarEngine = &grammarEngine{}
	if err := r.Prepare(canceled, failed); !errors.Is(err, context.Canceled) {
		t.Fatalf("Prepare() error = %v, want context.Canceled", err)
	}
	if failed.Tokens != nil || failed.MediaItems != nil || failed.Layout != nil || failed.Grammar != nil {
...
type cancellationIgnoringMediaModel struct {
	stubMediaModel
	cancel context.CancelFunc
}

func (m cancellationIgnoringMediaModel) PrepareMedia(ctx context.Context, segments []base.Segment) (*base.PreparedRequest, error) {
	prepared, err := m.stubMediaModel.PrepareMedia(ctx, segments)
	m.cancel()
	return prepared, err
}

func TestPrepareRejectsSuccessReturnedAfterCancellation(t *testing.T) {
	for _, initialNumPredict := range []int{0, 17} {
		t.Run(fmt.Sprintf("num_predict_%d", initialNumPredict), func(t *testing.T) {
			ctx, cancel := context.WithCancel(context.Background())
			r := mediaTestRunner(t)
			r.Model = cancellationIgnoringMediaModel{stubMediaModel: stubMediaModel{expansion: []int32{70, 71}, layout: "L"}, cancel: cancel}
			r.grammarEngine = &grammarEngine{}
			req := &Request{CompletionRequest: CompletionRequest{
				Prompt:  "[img-0]",
				Media:   []llm.MediaData{{ID: 0, Kind: llm.MediaKindImage, Data: []byte("img")}},
			...
			if err := r.Prepare(ctx, req); !errors.Is(err, context.Canceled) {
				t.Fatalf("Prepare() error = %v, want context.Canceled", err)
			}
			if req.Grammar != nil || req.Tokens != nil || req.MediaItems != nil || req.Layout != nil {
				t.Fatalf("canceled Prepare published state: %+v", req)
			}
			if req.Options.NumPredict != initialNumPredict {
				t.Fatalf("NumPredict = %d, want unchanged %d", req.Options.NumPredict, initialNumPredict)
			}
		})
	}
}
