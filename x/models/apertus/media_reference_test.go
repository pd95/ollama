...
func TestMediaCodeReference(t *testing.T) {
	...
	if err := thread.Do(context.Background(), func() error {
		...
		imageCodes, err := m.Vision.encode(context.Background(), imageInput.pixels, imageInput.width, imageInput.height)
		if err != nil {
			return err
		}
		mlx.Eval(imageCodes)
		...
		resizedCodes, err := m.Vision.encode(context.Background(), resizedInput.pixels, resizedInput.width, resizedInput.height)
		if err != nil {
			return err
		}
		...
		audioCodes, err := m.Audio.encode(context.Background(), samples)
		if err != nil {
			return err
		}
		mlx.Eval(audioCodes)
		...
		shortCodes, err := m.Audio.encode(context.Background(), []float32{float32(targetPeak), float32(-targetPeak / 2)})
		if err != nil {
			return err
		}
		mlx.Eval(shortCodes)
