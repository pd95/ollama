	...
	for _, quantized := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			if quantized {
				...
				assertGemma4QuantizedLinearType(t, m.EmbedVision.Projection, "mxfp8")
				assertGemma4QuantizedLinearType(t, m.EmbedAudio.Projection, "mxfp8")
				projected := m.EmbedAudio.Projection.Forward(mlx.Zeros(mlx.DTypeBFloat16, 1, 640))
				mlx.Eval(projected)
				if got, hidden := projected.Dims(), int(m.HiddenSize); len(got) != 2 || got[0] != 1 || got[1] != hidden {
					t.Fatalf("quantized unified audio projection shape = %v, want [1 %d]", got, hidden)
				}
			}
...
func TestGemma4UnifiedImporterOutputKeepsDirectAudioDense(t *testing.T) {
	useMLXTestThread(t)
	m, tensors := testGemma4UnifiedMediaModel(true)
	const path = "model.embed_audio.embedding_projection"
	weightName := path + ".weight"
	tensors[weightName] = testGemma4Array(64, 640)
	delete(tensors, weightName+"_scale")
	delete(tensors, weightName+"_qbias")
	delete(m.TensorQuant, weightName)

	if err := m.LoadWeights(tensors); err != nil {
		t.Fatal(err)
	}
	assertGemma4LinearStorage(t, m.EmbedAudio.Projection, false)
	assertGemma4LinearStorage(t, m.UnifiedVision.PatchDense, true)
	assertGemma4LinearStorage(t, m.EmbedVision.Projection, true)
}
...
func testGemma4TowerMediaModel(t *testing.T, quantized bool) (*Model, map[string]*mlx.Array) {
	...
	visionConfig := &VisionConfig{
		...
		RMSNormEps: 1e-6, DefaultOutputLength: 1, PatchSize: 8, PositionEmbeddingSize: 4,
	}
	...
	tensors["model.vision_tower.patch_embedder.position_embedding_table"] = testGemma4Array(2, 4, hidden)
...
func testGemma4UnifiedMediaModel(quantized bool) (*Model, map[string]*mlx.Array) {
	...
	tensors := testGemma4BaseTensors(hidden)
	visionConfig := &VisionConfig{
		ModelType: "gemma4_unified_vision", Unified: true, MMEmbedDim: hidden, MMPosembSize: 4,
		ModelPatchSize: 8, PatchSize: 8, PoolingKernelSize: 1, RMSNormEps: 1e-6, DefaultOutputLength: 4,
	}
	audioConfig := &AudioConfig{
	...
	putGemma4TestLinear(tensors, tensorQuant, "model.embed_vision.embedding_projection", hidden, hidden, quantized)
	putGemma4TestLinear(tensors, tensorQuant, "model.embed_audio.embedding_projection", hidden, 640, quantized)
