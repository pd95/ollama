...
type sdpaConfig struct {
	...
	// Optional attention sinks supplied by models such as GPT-OSS.
	sinks *mlx.Array
}
...
// WithSinks supplies optional attention sinks to the underlying MLX SDPA kernel.
func WithSinks(s *mlx.Array) SDPAOption {
	return func(c *sdpaConfig) { c.sinks = s }
}
...
func ScaledDotProductAttention(b *batch.Batch, q *mlx.Array, scale float32, opts ...SDPAOption) *mlx.Array {
	...
	if cached, ok := b.Memo.Get(inputs); ok {
		...
		return mlx.FastScaledDotProductAttentionWithSinks(q, k, v, scale, d.mode, d.arr, cfg.sinks)
	}
