...
func splitGateUpBiasTensor(td *safetensors.TensorData) ([]*safetensors.TensorData, error) {
	...
	gateRaw := encodeGPTOSSBF16(gateVals)
	upRaw := encodeGPTOSSBF16(upVals)
...
func decodeGPTOSSBF16(raw []byte) ([]float32, error) {
	if len(raw)%2 != 0 {
...
func encodeGPTOSSBF16(values []float32) []byte {
