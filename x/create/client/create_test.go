...
func writeClientSafetensors(t *testing.T, dir string, names ...string) {
	...
	for _, name := range names {
		...
		tensors = append(tensors, safetensors.NewTensorDataFromBytes(name, dtype, shape, clientSafetensorData(dtype, shape)))
	}
...
func writeClientSafetensorDescriptors(t *testing.T, dir string, descriptors map[string]gemma4metadata.TensorDescriptor) {
	...
	for name, descriptor := range descriptors {
		raw := clientSafetensorData(descriptor.Dtype, descriptor.Shape)
		if strings.EqualFold(descriptor.Dtype, "F32") && len(raw) == 4 {
			copy(raw, []byte{0, 0, 128, 63})
		}
...
func clientSafetensorData(dtype string, shape []int32) []byte {
	width := 1
	...
	return make([]byte, elements*width)
}
