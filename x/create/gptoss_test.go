...
func TestCreateGPTOSSNativeMXFP4MissingBiasIsRejectedWithoutRequest(t *testing.T) {
	...
	err := Create(context.Background(), "gptoss", dir, testPipelineOptions(), newCaptureStore(), func(context.Context, string, ManifestInfo) error {
		return nil
	}, func(string) {})
...
func TestCreatePipelineReportsGPTOSSMXFP4FileType(t *testing.T) {
	...
	opts.Quantize = "mxfp4"
	err := Create(context.Background(), "gptoss", dir, opts, newCaptureStore(), func(_ context.Context, _ string, info ManifestInfo) error {
		got = info.Class
		return nil
	}, func(string) {})
	if err != nil {
...
func mustDecodeBF16Tensor(t *testing.T, td *st.TensorData) []float32 {
	...
	values, err := decodeGPTOSSBF16(raw)
	if err != nil {
