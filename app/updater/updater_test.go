...
func TestStagedUpdateRejectsMalformedMetadata(t *testing.T) {
	oldStageDir := UpdateStageDir
	oldVerify := VerifyDownload
	defer func() {
		UpdateStageDir = oldStageDir
	...
	if staged, ok := StagedUpdate(); ok {
		t.Fatalf("malformed staged update = %+v, want rejected", staged)
	}
	if verifyCount.Load() != 0 {
		t.Fatalf("verification count = %d, want metadata rejection before verification", verifyCount.Load())
	}
	if _, err := os.Stat(bundle); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("malformed staged bundle was not removed: %v", err)
	}
}
