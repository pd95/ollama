...
func TestBuildChatRequestPreservesAudioAttachmentsAsMedia(t *testing.T) {
	...
	req, err := server.buildChatRequest(chat, "gemma4:12b-mlx-media-nvfp4", false, nil)
	if err != nil {
...
func TestCheckForUpdatesReturnsStructuredResults(t *testing.T) {
	...
	t.Run("up to date", func(t *testing.T) {
		...
		if strings.TrimSpace(recorder.Body.String()) != `{"status":"up_to_date","source":"official","channel":"stable"}` {
	...
	t.Run("ready", func(t *testing.T) {
		...
		if strings.TrimSpace(recorder.Body.String()) != `{"status":"ready","version":"v9.9.9","buildVersion":"9.9.9","source":"official","channel":"stable","releasePageUrl":"https://github.com/ollama/ollama/releases/tag/v9.9.9"}` {
			t.Fatalf("response = %s", recorder.Body.String())
		}
		if trayCalls.Load() != 1 {
...
func TestRequestedUpdateSource(t *testing.T) {
	oldManifestURL := updater.UpdateManifestURL
	oldSource := updater.UpdateSource
	updater.UpdateManifestURL = "https://updates.example.invalid/manifest.json"
	updater.UpdateSource = updater.MLXPreviewSource
	t.Cleanup(func() {
		updater.UpdateManifestURL = oldManifestURL
		updater.UpdateSource = oldSource
	})
	...
	for _, test := range []struct {
		...
		{"compatibility alias", "", updater.MLXPreviewSource, false},
		{"preview", `{"source":"mlx-preview"}`, updater.MLXPreviewSource, false},
		{"official", `{"source":"official"}`, updater.OfficialUpdateSource, false},
		{"unsupported", `{"source":"nightly"}`, "", true},
		{"malformed", `{`, "", true},
	} {
		t.Run(test.name, func(t *testing.T) {
			request := httptest.NewRequest(http.MethodPost, "/api/v1/update/check", strings.NewReader(test.body))
			got, err := requestedUpdateSource(request)
			if (err != nil) != test.err || got != test.want {
				t.Fatalf("source = %q, error = %v; want %q, error %v", got, err, test.want, test.err)
			}
		})
	}
}
...
func TestInstallUpdateIsSingleFlightAndReportsResults(t *testing.T) {
	...
	server := &Server{InstallUpdateFunc: func(string) error {
		close(started)
		<-release
	...
	failure := &Server{InstallUpdateFunc: func(string) error { return errors.New("permission denied") }}
